    ...
    fn compute_pi_polynomials(hat_P: &[E::Fr], point: &[E::Fr]) -> Vec<Vec<E::Fr>> {
        ...
        for i in 0..point.len() - 1 {
            ...
            (0..Pi_len)
                .into_par_iter()
                .map(|j| {
                    point[point.len() - i - 1] * (polys[i][2 * j + 1] - polys[i][2 * j])
                        + polys[i][2 * j]
                })
    ...
    E::G1Affine: TranscriptReprTrait<E::G1>,
{
    ...
    fn verify(
        ...
        let pairing_result =
            E::multi_miller_loop(pairing_input_refs.as_slice()).final_exponentiation();
...
mod tests {
    ...
    use crate::{
        provider::{
            ...
            util::{
                iterators::DoubleEndedIteratorExt as _, test_utils::prove_verify_from_num_vars,
            },
        },
        spartan::powers,
    ...
    fn test_commitment_to_k_polynomial_correctness(
        ...
        let C_Q = <<crate::provider::Bn256EngineKZG as NovaEngine>::CE as CommitmentEngineTrait<
            NE,
        >>::commit(ck, &Q_x.coeffs)
        .comm
        .to_affine();
    ...
    fn test_hyperkzg_shplonk_eval() {
        ...
        let test_inner = |point: Vec<Fr>, eval: Fr| -> Result<(), NovaError> {
            ...
            let proof =
                EvaluationEngine::<E, NE>::prove(&ck, &pk, &mut tr, &C, &poly, &point, &eval)
                    .unwrap();
