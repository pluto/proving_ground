...
/// Note: this is non-hiding, which is why we will implement traits on this
/// token struct, as we expect to have several impls for the trait pegged on the
/// same instance of a pairing::Engine.
    ...
    E::G1: DlogGroup<AffineExt = E::G1Affine, ScalarExt = E::Fr>,
{
    fn commit_offset(
        ...
        // We can avoid some scalar multiplications if 'scalars' contains a lot of
        // leading zeroes using offset, that points where non-zero scalars
        // start.
...
// TODO: important, we need a better way to handle that the commitment key
// should be 2^max_degree sized, see the runtime error in commit() below
...
/// Note: this is non-hiding, which is why we will implement the
/// `EvaluationEngineTrait` on this token struct, as we will have several impls
/// for the trait pegged on the same instance of a `pairing::Engine`.
#[allow(clippy::upper_case_acronyms)]
    ...
    <E::G1 as Group>::Base: TranscriptReprTrait<E::G1>,
{
    ...
    /// On input a polynomial `poly` and a point `point`, outputs a proof for
    /// the same.
    fn open(
        pp: &impl Borrow<ZMProverKey<E>>,
        comm: &ZMCommitment<E>,
...
/// Computes the quotient polynomials of a given multilinear polynomial with
/// respect to a specific input point.
///
/// Given a multilinear polynomial `poly` and a point `point`, this function
/// calculates the quotient polynomials `q_k` and the evaluation at `point`,
/// such that:
///
/// ```text
...
/// ```
///
/// where `poly(point)` is the evaluation of `poly` at `point`, and each `q_k`
/// is a polynomial in `k` variables.
///
/// Since our evaluations are presented in order reverse from the coefficients,
/// if we want to interpret index `q_k` to be the k-th coefficient in the
/// polynomials returned here, the equality that holds is:
///
/// ```text
...
/// Computes some key terms necessary for computing the partially evaluated
/// univariate ZM polynomial
fn eval_and_quotient_scalars<F: Field>(y: F, x: F, z: F, point: &[F]) -> (F, (Vec<F>, Vec<F>)) {
    ...
    let squares_of_x = iter::successors(Some(x), |&x| Some(x.square()))
        .take(num_vars + 1)
        .collect::<Vec<_>>();
    // offsets_of_x = [Π_{j=i}^{num_vars-1} x^(2^j), i ∈ [0, num_vars-1]] =
    // [x^(2^num_vars - d_i - 1), i ∈ [0, num_vars-1]]
    let offsets_of_x = {
    ...
    // Note Φ_(n-i)(x^(2^i)) = (x^(2^i))^(2^(n-i) - 1) / (x^(2^i) - 1) =
    // (x^(2^num_vars) - 1) / (x^(2^i) - 1) = vs[i]      Φ_(n-i-1)(x^(2^(i+1)))
    // = (x^(2^(i+1)))^(2^(n-i-1)) - 1 / (x^(2^(i+1)) - 1) = (x^(2^num_vars) - 1) /
    // (x^(2^(i+1)) - 1) = vs[i+1]
    let vs = {
    ...
    // q_scalars = [- (y^i * x^(2^num_vars - d_i - 1) + z * (x^(2^i) * vs[i+1] - u_i
    // * vs[i])), i ∈ [0, num_vars-1]]           = [- (y^i * x^(2^num_vars - d_i
    // - 1) + z * (x^(2^i) * Φ_(n-i-1)(x^(2^(i+1))) - u_i * Φ_(n-i)(x^(2^i)))), i ∈
    // [0, num_vars-1]]
    #[allow(clippy::disallowed_methods)]
...
mod test {
    ...
    fn test_quotients() {
        ...
        // except for our inversion of coefficient order in polynomials and points (see
        // below)
        ...
        for (k, q_k) in quotients.iter().enumerate() {
            ...
            // the following looks weird because the quotient polynomials are coefficiented
            // in reverse order from evaluation IOW in 'normal evaluation order'
            // this should be let z_partial = &z_challenge[..k];
            let z_partial = &z_challenge[z_challenge.len() - k..];

            let q_k_eval = q_k_poly.evaluate(z_partial);
            // the following looks weird because the quotient polynomials are coefficiented
            // in reverse order from evaluation IOW in 'normal evaluation order'
            // this should be result -= (z_challenge[k] - u_challenge[k]) *
            // q_k_eval;
            result -= (z_challenge[z_challenge.len() - k - 1]
                - u_challenge[z_challenge.len() - k - 1])
                * q_k_eval;
        }

        // Assert that the result is zero, which verifies the correctness of the
        // quotients
        assert!(
    ...
    fn test_batched_lifted_degree_quotient() {
        ...
        // Now explicitly define q_k_lifted = X^{N-2^k} * q_k and compute the expected
        // batched result
    ...
    fn phi<F: PrimeField>(challenge: F, n: usize) -> F {
        ...
        for _ in 0..length {
            result += current;
            current *= challenge; // Increment the power of x for the next
                                  // iteration
        }
    ...
    fn test_partially_evaluated_quotient_z() {
        ...
        for k in 0..num_vars {
            ...
            let mut scalar = x_pow_2k * phi(x_pow_2kp1, num_vars - k - 1)
                - u_rev[k] * phi(x_pow_2k, num_vars - k);
            scalar *= z_challenge;
            scalar *= -Scalar::ONE;
        ...
        E::Fr: PrimeFieldBits,
    {
        ...
        // We use following filter to optimise MSM for cases where scalars contain a lot
        // of zeroes
        ...
        E::Fr: PrimeFieldBits,
    {
        ...
        let multilinear_poly =
            MultilinearPolynomial::<E::Fr>::random(num_vars_multilinear, &mut rng);
        ...
        let (q_hat, offset) =
            batched_lifted_degree_quotient(E::Fr::random(&mut rng), &quotients_polys);
