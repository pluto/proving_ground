...
//!    2. R1CS Instance u is folded into Ui[augmented_circuit_index] correctly;
//!       just like Nova IVC.
//!    3. (optional by F logic) F circuit might check `program_counter_{i}`
//!       invoked current F circuit is legal or not.
//!    3. F circuit produce `program_counter_{i+1}` and sent to next round to
//!       optionally constraint the next F' argumented circuit.
...
use crate::{
    ...
    gadgets::{
        alloc_num_equals, alloc_scalar_as_base, alloc_zero,
...
impl<'a, E: Engine, SC: EnforcingStepCircuit<E::Base>> SuperNovaAugmentedCircuit<'a, E, SC> {
    ...
    pub fn synthesize<CS: ConstraintSystem<<E as Engine>::Base>>(
        ...
        let is_base_case =
            alloc_num_equals(cs.namespace(|| "Check if base case"), &i.clone(), &zero)?;
