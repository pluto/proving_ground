...
use crate::{
    ...
    gadgets::{
        alloc_bignat_constant, f_to_nat, le_bits_to_num, AllocatedPoint,
...
impl<E: Engine> AllocatedCycleFoldInstance<E> {
    pub fn alloc<CS: ConstraintSystem<E::Base>>(
        ...
        let X: [BigNat<E::Base>; NIO_CYCLE_FOLD] = (0..NIO_CYCLE_FOLD)
            ...
            .try_into()
            .map_err(|err: Vec<_>| {
                SynthesisError::IncompatibleLengthVector(format!(
                    "{} != {NIO_CYCLE_FOLD}",
                    err.len()
                ))
            })?;
            ...
            .try_for_each(|(io_idx, x)| -> Result<(), SynthesisError> {
                x.as_limbs().iter().enumerate().try_for_each(
                    |(limb_idx, limb)| -> Result<(), SynthesisError> {
                        ro.absorb(&limb.as_allocated_num(cs.namespace(|| {
                            format!("convert limb {limb_idx} of X[{io_idx}] to num")
                        }))?);
                        Ok(())
                    },
                )
...
impl<E: Engine> AllocatedCycleFoldData<E> {
        ...
        CS: ConstraintSystem<E::Base>,
    {
        ...
        for (idx, (X, x)) in self.U.X.iter().zip_eq(self.u.X.iter()).enumerate() {
            ...
            let r_new = X.add(&r)?;
            let X_i_fold =
                r_new.red_mod(cs.namespace(|| format!("reduce folded X[{idx}]")), &m_bn)?;
...
pub mod emulated {
        ...
        G: Group,
    {
            ...
            CS: ConstraintSystem<G::Base>,
        {
            ...
            let is_infinity_num: AllocatedNum<G::Base> =
                AllocatedNum::alloc(cs.namespace(|| "is_infinity"), || {
                    self.is_infinity.get_value().map_or(
        ...
        pub fn default<CS: ConstraintSystem<G::Base>>(
            ...
            let is_infinity =
                AllocatedBit::alloc(cs.namespace(|| "allocate is_infinity"), Some(true))?;
            cs.enforce(
                || "is_infinity = 1",
                |lc| lc,
                |lc| lc,
        ...
        E: Engine,
    {
        ...
        pub fn fold_with_r1cs<CS: ConstraintSystem<<E as Engine>::Base>>(
            ...
            let x0_fold =
                AllocatedNum::alloc(cs.namespace(|| "x0"), || {
                    Ok(*self.x0.get_value().get()?
                        + *r.get_value().get()? * *u_x0.get_value().get()?)
                })?;
            cs.enforce(
                || "x0_fold = x0 + r * u_x0",
            ...
            let x1_fold =
                AllocatedNum::alloc(cs.namespace(|| "x1"), || {
                    Ok(*self.x1.get_value().get()?
                        + *r.get_value().get()? * *u_x1.get_value().get()?)
                })?;
            cs.enforce(
                || "x1_fold = x1 + r * u_x1",
                |lc| lc + r.get_variable(),
        ...
        pub fn default<CS: ConstraintSystem<<E as Engine>::Base>>(
            ...
            let comm_W = AllocatedEmulPoint::default(
                cs.namespace(|| "default comm_W"),
                limb_width,
                n_limbs,
            )?;
